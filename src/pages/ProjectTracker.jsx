...
import StageSteps from "../components/StageSteps.jsx";
import StatusPill from "../components/StatusPill.jsx";
import { Card, DateValue, Field, PageHead, Section } from "../components/Layout.jsx";
...
export default function ProjectTracker() {
  ...
  return (
    <>
      ...
      <Card className="mt-6 grid gap-6 p-5 lg:grid-cols-[auto_1fr] lg:gap-10 lg:p-6">
        <div className="flex items-center gap-5">
          ...
          />
          <div>
            <div className="text-muted text-[11.5px] tabular-nums">
        ...
        <dl className="grid grid-cols-2 gap-x-6 gap-y-4 self-start sm:grid-cols-3">
          <Field label="Assigned To">{project.employee || "not recorded"}</Field>
              ...
              <tr>
                {HEADERS.map((header) => (
                  <th
                    key={header}
                    scope="col"
                    className="border-line bg-surface-2 text-muted border-b px-4.5 py-3 text-left text-[10.5px] font-bold tracking-widest whitespace-nowrap uppercase"
                  >
                    {header}
                  </th>
                ))}
              ...
              <tbody key={`${group.type}-${groupIndex}`}>
                <tr>
                  <th
                    ...
                    className="border-line-2 text-accent-ink border-t px-4.5 pt-4 pb-2 text-left text-[10.5px] font-bold tracking-widest uppercase"
                  >
                    {group.type}
                ...
                {group.items.map((item, index) => {
                  return (
                    <tr key={`${item.item}-${index}`} className="border-line-2 hover:bg-surface-2 border-t align-top">
                      ...
                      <td className="text-ink-2 px-4.5 py-3.5 text-[12.5px]">
